<template>
  <div class="app-container">
      <Permission_add  @addSaveTodo="addSaveTodo"></Permission_add>
      <Permission_edit @addSaveTodo="addSaveTodo"></Permission_edit>
      ...
      <el-form :model="listQuery" ref="listQuery" :inline="true" class="demo-form-inline" >
          <div align="center" >
              <el-form-item prop="name" label="菜单名称：">
                  <el-input  placeholder="请输入菜单名称" v-model="listQuery.name"></el-input>
              </el-form-item>

              <el-form-item  prop="dob" label="创建日期" >
      ...
      <el-row>
          ...
          <el-col :span="20">
              <el-table ref="multipleTable" tooltip-effect="dark" :data="tableData"  @selection-change="handleSelectionChange" highlight-current-row  fit border  size="mini"  style="width: 100%" max-height="700"
                  ...
                  <el-table-column prop="remark" label="菜单备注" align="center" show-overflow-tooltip> </el-table-column>
...
<script>
    import Permission_add from './Permission_add.vue';
    import Permission_edit from './Permission_edit.vue';
    import {StrToGMT,GMTToStr} from '../../utils/times.js';
    export default {
        components:{ Permission_add,Permission_edit } ,// 注册局部组件
        data() {
            const data = [{
                id: 1,
                label: '一级 1',
        ...
        methods:{
            ...
            getuser(){
                this.$axios.post('/api/sys_permission/queryList',this.listQuery).then(  // sos 全局引用 方法 在main.js中 加入import axios from 'axios' 和 Vue.prototype.$ajax = axios
                    (res) => {
            ...
            handleDelete: function (index,row) {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {type: 'warning'}).then(() => {
                    this.$axios.delete('api/sys_permission/DELETE/'+row.id).then(function(res){
            ...
            batchDelete(){ // 批量删除
                ...
                this.$confirm('确认删除多条记录吗?', '提示', {type: 'warning'}).then(() => {
                    _this.$axios.delete('api/sys_permission/batchDelete/'+ids).then(function(res){
                        _this.$message({message: '删除成功',type: 'success'}); //
                        _this.getuser();// 删除成功后 刷新当前组件
                    }) }).catch(() => { });// this.tableData.splice(index, 1);// 删除本地元素
            },
