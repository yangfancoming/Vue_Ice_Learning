<template>
  ...
  <el-dialog title="新增" width="30%"   center   :visible.sync="add_show" :before-close="beforeClose">
    <el-form  :model="role"  ref="ruleForm"  class="demo-form-inline" >
        <el-form-item prop="name" label="菜单名称" :label-width="formLabelWidth">
            <el-input v-model="role.name"  placeholder="请输入 角色名称"></el-input>
        </el-form-item>
        <el-form-item prop="url" label="菜单地址" :label-width="formLabelWidth">
            <el-input v-model.number="role.url"  placeholder="请输入 显示顺序"></el-input>
        </el-form-item>
...
//                    this.role.dob =  GMTToStr(this.role.dob); // 1. sos 转换
                    this.$axios.post('api/sys_permission/POST', this.role).then(function (res) {
